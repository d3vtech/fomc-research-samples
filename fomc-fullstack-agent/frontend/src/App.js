...
export default function App() {
    ...
    const retryWithBackoff = async (fn, maxRetries = 10, maxDuration = 120000 // 2 minutes
    ) => {
    ...
    const extractDataFromSSE = (data) => {
        try {
            ...
            if (parsed.content && parsed.content.parts) {
                ...
                const functionCallPart = parsed.content.parts.find((part) => part.functionCall);
                if (functionCallPart) {
                    functionCall = functionCallPart.functionCall;
                }
                // Check for function responses
                const functionResponsePart = parsed.content.parts.find((part) => part.functionResponse);
    ...
    const processSseEventData = (jsonData, aiMessageId) => {
        ...
        if (textParts.length > 0 && agent !== "report_composer_with_citations") {
            if (agent !== "interactive_planner_agent") {
                ...
                for (const text of textParts) {
                    ...
                    setMessages(prev => prev.map(msg => msg.id === aiMessageId ? { ...msg, content: accumulatedTextRef.current.trim(), agent: currentAgentRef.current || msg.agent } : msg));
    ...
    const handleSubmit = useCallback(async (query, model, effort) => {
        if (!query.trim())
            return;
        setIsLoading(true);
        try {
    ...
    const BackendLoadingScreen = () => (_jsx("div", { className: "flex-1 flex flex-col items-center justify-center p-4 overflow-hidden relative", children: _jsx("div", { className: "w-full max-w-2xl z-10\n                      bg-neutral-900/50 backdrop-blur-md \n                      p-8 rounded-2xl border border-neutral-700 \n                      shadow-2xl shadow-black/60", children: _jsxs("div", { className: "text-center space-y-6", children: [_jsx("h1", { className: "text-4xl font-bold text-white flex items-center justify-center gap-3", children: "\u2728 Gemini FullStack - ADK \uD83D\uDE80" }), _jsxs("div", { className: "flex flex-col items-center space-y-4", children: [_jsxs("div", { className: "relative", children: [_jsx("div", { className: "w-16 h-16 border-4 border-neutral-600 border-t-blue-500 rounded-full animate-spin" }), _jsx("div", { className: "absolute inset-0 w-16 h-16 border-4 border-transparent border-r-purple-500 rounded-full animate-spin", style: { animationDirection: 'reverse', animationDuration: '1.5s' } })] }), _jsxs("div", { className: "space-y-2", children: [_jsx("p", { className: "text-xl text-neutral-300", children: "Waiting for backend to be ready..." }), _jsx("p", { className: "text-sm text-neutral-400", children: "This may take a moment on first startup" })] }), _jsxs("div", { className: "flex space-x-1", children: [_jsx("div", { className: "w-2 h-2 bg-blue-500 rounded-full animate-bounce", style: { animationDelay: '0ms' } }), _jsx("div", { className: "w-2 h-2 bg-purple-500 rounded-full animate-bounce", style: { animationDelay: '150ms' } }), _jsx("div", { className: "w-2 h-2 bg-pink-500 rounded-full animate-bounce", style: { animationDelay: '300ms' } })] })] })] }) }) }));
    return (_jsx("div", { className: "flex h-screen bg-neutral-800 text-neutral-100 font-sans antialiased", children: _jsx("main", { className: "flex-1 flex flex-col overflow-hidden w-full", children: _jsx("div", { className: `flex-1 overflow-y-auto ${(messages.length === 0 || isCheckingBackend) ? "flex" : ""}`, children: isCheckingBackend ? (_jsx(BackendLoadingScreen, {})) : !isBackendReady ? (_jsx("div", { className: "flex-1 flex flex-col items-center justify-center p-4", children: _jsxs("div", { className: "text-center space-y-4", children: [_jsx("h2", { className: "text-2xl font-bold text-red-400", children: "Backend Unavailable" }), _jsx("p", { className: "text-neutral-300", children: "Unable to connect to backend services at localhost:8000" }), _jsx("button", { onClick: () => window.location.reload(), className: "px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors", children: "Retry" })] }) })) : messages.length === 0 ? (_jsx(WelcomeScreen, { handleSubmit: handleSubmit, isLoading: isLoading, onCancel: handleCancel })) : (_jsx(ChatMessagesView, { messages: messages, isLoading: isLoading, scrollAreaRef: scrollAreaRef, onSubmit: handleSubmit, onCancel: handleCancel, displayData: displayData, messageEvents: messageEvents, websiteCount: websiteCount })) }) }) }));
}
